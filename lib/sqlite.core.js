...
plugin.exec = function(method, options, success, error, sqlIndex) {
  var successLog, errorLog;
  var sqls = options?.executes ?? [{}];
  var sql = sqls.map(s => s.sql || "??").join("\n... ");

  if (sqlIndex === undefined) {
    sqlIndex = 0;
  }
  ...
  if (plugin.sqlitePlugin.DEBUG) {
    console.log(`SQLite ${sqlIndex} ${startTime} START ${sql}`);
  }

  successLog = function(result) {
    console.log(`SQLite ${sqlIndex} ${Date.now() - startTime} END ${sql}`);
    if (result && result[0] && result[0].result) {
      console.log(
        `Native ${result[0].result.sqlIndex} ${result[0].result.sqlTiming /
          1000} END`
      );
    }
    success(...arguments);
  ...
  if (method === "backgroundExecuteSqlBatch") {
    NativeModules["SQLite"][method](options, sqlIndex, successLog, errorLog);
  } else {
    NativeModules["SQLite"][method](options, successLog, errorLog);
  }
};
...
SQLitePlugin.prototype.open = function(success, error) {
  ...
  if (this.dbname in this.openDBs) {
    ...
    this.openDBs[this.dbname] = DB_STATE_INIT;
    plugin.exec("open", this.openargs, opensuccesscb, openerrorcb, ++sqlCount);
  }
};
...
SQLitePlugin.prototype.close = function(success, error) {
  ...
  if (this.dbname in this.openDBs) {
    ...
    plugin.exec("close", { path: this.dbname }, mysuccess, myerror, ++sqlCount);
  } else {
...
SQLiteFactory.prototype.deleteDatabase = function(first, success, error) {
  ...
  plugin.exec("delete", args, success, error, ++sqlCount);
};
